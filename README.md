...
* Online scoring using saved machine learning model through curl 
* Interact with IBM Db2 Event Store through its REST API
* Running IBM Streams and Remote Access Applications with IBM Db2 Event Store
...
Clone the `db2-event-store-IoT-Analytics` locally. In a terminal, run:

- ```
  git clone https://github.com/IBMProjectEventStore/db2-event-store-IoT-Analytics
  ```
...
>
>  ./load.sh --user <user_name> --connstring \<host IP\>:1101 --filename ./data/sample_IOT_table.csv --dbname TESTDB --tablename IOT_TEMP --numrowsperbatch 100000
...
See the notebook with example output here: [notebook examples with result](notebooks_with_result)
