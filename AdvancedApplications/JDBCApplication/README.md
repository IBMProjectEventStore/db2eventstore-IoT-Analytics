...
Follow these steps when running the JDBC java sample application ExampleJDBCApp.java outide of the container. If you are running inside the container you 
can start at step 5. 
...
5. In runJDBCExample, make sure the client jar defined for ESLIB is the correct version. The container currently uses ibm-db2-eventstore-client-spark-2.4.6-2.0.1.0.jar (If you are not running inside the container you need to obtain the client jar from Maven with this [link](https://mvnrepository.com/artifact/com.ibm.event/ibm-db2-eventstore-client-spark-2.4.6)). This JAR includes the Db2 Standard JDBC client. Alternatively, you can download the Db2 JDBC client driver from the [IBM Data Server Client Packages page under the IBM Support website](https://www-01.ibm.com/support/docview.wss?uid=swg21385217):
